...
class Solution:
    ...
    def canCross(self, stones: List[int]) -> bool:
        ...
        @lru_cache(2**10)
        def backtrack(i, speed):
            if i not in stoneSet:
                return False
            if i == stones:
                return True
            if speed == 0:
                return backtrack(i + 1, 1)
            elif speed == 1:
                return backtrack(i + 1, 1) or backtrack(i + 2, 2)
            else:
                return backtrack(i + speed, speed) or backtrack(
                    i + speed - 1, speed - 1) or backtrack(i + speed + 1, speed + 1)
