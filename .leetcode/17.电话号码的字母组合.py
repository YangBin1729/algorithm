...
class Solution:
    ...
    def letterCombinations_1(self, digits: str) -> List[str]:
        ...
        return list(
            map(lambda x: ''.join(x),
                product(*(self.mapping[d] for d in digits)))) if digits else []
    ...
    def letterCombinations_2(self, digits: str) -> List[str]:
        ...
        res = [''] if digits else []
        for d in digits:
            res = [r + l for r in res for l in self.mapping[d]]
        return res
    ...
    def letterCombinations(self, digits: str) -> List[str]:
        ...
        def backtrack(combination, next_digits):
            """
            combination: 已经产生的组合
            next_digits：接下来准备输入的数字
            """
            if len(next_digits) == 0:
                res.append(combination)
            else:
                for letter in self.mapping[next_digits[0]]:
